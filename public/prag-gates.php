...
<!--    <title>UHT</title>-->
    ...
    <script>
        ...
        !function (i, s) {
            var e = "0.7.10", o = "", r = "?", n = "function", a = "undefined", t = "object", w = "string", l = "major",
                d = "model", p = "name", c = "type", u = "vendor", m = "version", f = "architecture", b = "console",
                g = "mobile", h = "tablet", v = "smarttv", x = "wearable", y = "embedded", k = {
                    extend: function (i, s) {
                        var e = {};
                    ...
                    rgx: function () {
                        for (var i, e, o, r, w, l, d, p = 0, c = arguments; p < c.length && !l;) {
                            ...
                            for (e = o = 0; e < u.length && !l;) if (l = u[e++].exec(this.getUA())) for (r = 0; r < m.length; r++) d = l[++o], w = m[r], typeof w === t && w.length > 0 ? 2 == w.length ? i[w[0]] = typeof w[1] == n ? w[1].call(this, d) : w[1] : 3 == w.length ? i[w[0]] = typeof w[1] !== n || w[1].exec && w[1].test ? d ? d.replace(w[1], w[2]) : s : d ? w[1].call(this, d, w[2]) : s : 4 == w.length &&
                                (i[w[0]] = d ? w[3].call(this, d.replace(w[1], w[2])) : s) : i[w] = d ? d : s;
                            p += 2
                        }
                        return i
                    }, str: function (i, e) {
                        for (var o in e) if (typeof e[o] === t && e[o].length > 0) for (var n = 0; n < e[o].length; n++) {
                    ...
                    device: {
                        amazon: {model: {"Fire Phone": ["SD", "KF"]}}, sprint: {
                            model: {"Evo Shift 4G": "7373KT"}, vendor: {
                                HTC: "APA",
                                Sprint: "Sprint"
                            }
                        }
                    ...
                    browser: [[/(opera\smini)\/([\w\.-]+)/i, /(opera\s[mobiletab]+).+version\/([\w\.-]+)/i, /(opera).+version\/([\w\.]+)/i, /(opera)[\/\s]+([\w\.]+)/i], [p, m], [/(OPiOS)[\/\s]+([\w\.]+)/i], [[p, "Opera Mini"], m], [/\s(opr)\/([\w\.]+)/i], [[p, "Opera"], m], [/(kindle)\/([\w\.]+)/i, /(lunascape|maxthon|netfront|jasmine|blazer)[\/\s]?([\w\.]+)*/i,
                        ...
                        [m, [p, "Mobile Safari"]], [/version\/([\w\.]+).+?(mobile\s?safari|safari)/i], [m, p], [/webkit.+?(mobile\s?safari|safari)(\/[\w\.]+)/i], [p, [m, A.str, E.browser.oldsafari.version]], [/(konqueror)\/([\w\.]+)/i, /(webkit|khtml)\/([\w\.]+)/i], [p, m], [/(navigator|netscape)\/([\w\.-]+)/i], [[p, "Netscape"], m], [/(swiftfox)/i, /(icedragon|iceweasel|camino|chimera|fennec|maemo\sbrowser|minimo|conkeror)[\/\s]?([\w\.\+]+)/i, /(firefox|seamonkey|k-meleon|icecat|iceape|firebird|phoenix)\/([\w\.-]+)/i, /(mozilla)\/([\w\.]+).+rv\:.+gecko\/\d+/i,
                            /(polaris|lynx|dillo|icab|doris|amaya|w3m|netsurf|sleipnir)[\/\s]?([\w\.]+)/i, /(links)\s\(([\w\.]+)/i, /(gobrowser)\/?([\w\.]+)*/i, /(ice\s?browser)\/v?([\w\._]+)/i, /(mosaic)[\/\s]([\w\.]+)/i], [p, m]],
                    cpu: [[/(?:(amd|x(?:(?:86|64)[_-])?|wow|win)64)[;\)]/i], [[f, "amd64"]], [/(ia32(?=;))/i], [[f, k.lowerize]], [/((?:i[346]|x)86)[;\)]/i], [[f, "ia32"]], [/windows\s(ce|mobile);\sppc;/i], [[f, "arm"]], [/((?:ppc|powerpc)(?:64)?)(?:\smac|;|\))/i], [[f, /ower/, "", k.lowerize]], [/(sun4\w)[;\)]/i], [[f, "sparc"]], [/((?:avr32|ia64(?=;))|68k(?=\))|arm(?:64|(?=v\d+;))|(?=atmel\s)avr|(?:irix|mips|sparc)(?:64)?(?=;)|pa-risc)/i],
                        [[f, k.lowerize]]],
                    ...
                    os: [[/microsoft\s(windows)\s(vista|xp)/i], [p, m], [/(windows)\snt\s6\.2;\s(arm)/i, /(windows\sphone(?:\sos)*|windows\smobile|windows)[\s\/]?([ntce\d\.\s]+\w)/i], [p, [m, A.str, E.os.windows.version]], [/(win(?=3|9|n)|win\s9x\s)([nt\d\.]+)/i],
                        ...
                        [[p, "Mac OS"], [m, /_/g, "."]], [/((?:open)?solaris)[\/\s-]?([\w\.]+)*/i, /(haiku)\s(\w+)/i, /(aix)\s((\d)(?=\.|\)|\s)[\w\.]*)*/i, /(plan\s9|minix|beos|os\/2|amigaos|morphos|risc\sos|openvms)/i, /(unix)\s?([\w\.]+)*/i], [p, m]]
                }, T = function (s, e) {
            ...
            T.VERSION = e, T.BROWSER = {NAME: p, MAJOR: l, VERSION: m}, T.CPU = {ARCHITECTURE: f}, T.DEVICE =
                {
                    MODEL: d,
                    VENDOR: u,
                    TYPE: c,
                    ...
                    WEARABLE: x,
                    EMBEDDED: y
                }, T.ENGINE = {NAME: p, VERSION: m}, T.OS = {
                NAME: p,
                VERSION: m
            }, typeof exports !== a ? (typeof module !== a && module.exports && (exports = module.exports = T), exports.UAParser2 = T) : typeof define === n && define.amd ? define("ua-parser-js", [], function () {
                return T
            }) : i.UAParser2 = T;
        ...
        var UHT_DEVICE_TYPE = {MOBILE: false, DESKTOP: false};
        var UHT_FRAME = false;
        ...
        UHTConsole.GetReplacement = function (methodIdx) {
            return function () {
                var stringARGS = [];
                for (var i = 0; i < arguments.length; i++) if (arguments[i] != null) stringARGS.push(arguments[i].toString());
        ...
        UHTConsole.AllowToWrite = function (allowToWrite) {
            ...
            for (var i = 0; i < UHTConsole.methods.length; ++i) {
                ...
                if (!allowToWrite) if (UHTConsole.replacement[name] == null) UHTConsole.replacement[name] = UHTConsole.GetReplacement(i);
        ...
        Loader.LoadWURFL = function () {
            var wurflURL = "//wurfl.io/wurfl.js"; //"https://pragma-shared.mobule.games/wurfl.js";
        ...
        Loader.WURFLLoadHandler = function () {
            ...
            if (WURFL.complete_device_name != undefined) for (var id in LowEndDeviceIdentifiers) if (WURFL.complete_device_name.indexOf(LowEndDeviceIdentifiers[id]) >= 0) {
                UHT_LOW_END_DEVICE = true;
                break
            }
            console.log("WURFL loaded");
            UHT_DEVICE_TYPE = {MOBILE: WURFL.is_mobile, DESKTOP: !WURFL.is_mobile};
            Loader.SetExtraInfo();
        ...
        Loader.WURFLErrorHandler = function () {
            ...
            UHT_DEVICE_TYPE = {MOBILE: mobile, DESKTOP: !mobile};
            Loader.SetExtraInfo();
        ...
        Loader.SetExtraInfo = function () {
            ...
            if (device != undefined) for (var id in LowEndDeviceIdentifiers) if (device.indexOf(LowEndDeviceIdentifiers[id]) >= 0) {
                UHT_LOW_END_DEVICE = true;
                break
            }
        ...
        Loader.Listener = function (json) {
            ...
            if (params["common"] == "EVT_GET_CONFIGURATION") {
                ...
                var pathParts = [];
                for (var i = 0; i < tmp.length; ++i) if (tmp[i].length > 0) pathParts.push(tmp[i]);
        ...
        Loader.Start = function () {
            ...
            if (online) {
                ...
                sendToAdapter(JSON.stringify({common: "EVT_GET_CONFIGURATION", type: "html5"}))
        ...
        if (location.href.indexOf("WURFL_NOT_ALLOWED") > -1) Loader.WURFLErrorHandler(); else setTimeout(Loader.LoadWURFL, 0);
        ...
        UHTLogotype.HandleResize = function () {
            ...
            var r2 = w / h;
            if (UHTLogotype.data["fit"] == "shrink") if (r2 < r1) sh = "100%"; else sw = "100%"; else if (r2 < r1) sw = "100%"; else sh = "100%";
        ...
        UHTLogotype.GameLoadingStarted = function () {
            ...
            if (UHTLogotype.timer > 0) {
                ...
                if (dt <= 0) UHTLogotype.HideLogo(); else UHTLogotype.DelayHideLogo(dt)
            }
        };
        ...
        UHTLogotype.UpdateStyle = function (add, remove) {
            ...
            var cls = [];
            for (var i = 0; i < split.length; ++i) if (split[i].length > 0 && split[i] != remove) cls.push(split[i]);
            cls.push(add);
    ...
    <script type="text/javascript"
            src="https://demogamesfree.pragmaticplay.net/gs2c/common/js/html5-script-external.js"></script>
    <script type="text/javascript">
        Html5GameManager.init({
            contextPath: "/gs2c",
            cashierUrl: "",
            lobbyUrl: "javascript:window.top.postMessage('GT:EXIT','*')",
            mobileCashierUrl: "",
            mobileLobbyUrl: "",
            gameConfig: '{"datapath":"https://pragma-shared.mobule.games/gs2c/common/games-html5/games/vs/vs20fparty2/","LOGOUT":"/gs2c/logout.do","REGULATION":"https://pragma-shared.mobule.games/gs2c/regulation/process.do?symbol=vs20fparty2","showRealCash":"1","statisticsURL":"https://pragma-test.mobule.games/gs2c/stats.do","accountType":"R","jurisdiction":"MT","demoMode":"0","RELOAD_JACKPOT":"/gs2c/jackpot/reload.do","clock":"0","styleName":"generic","SETTINGS":"/gs2c/saveSettings.do","mgckey":"stylename@generic~SESSION@alkapon-108556-8a68b531-bbdc-441d-81fa-f8060c282091","gameService":"https://pragma-test.mobule.games/gs2c/v3/gameService","RELOAD_BALANCE":"/gs2c/reloadBalance.do","currencyOriginal":"BRL","vendor":"T","extend_events":"1","currency":"BRL","lang":"pt","CLOSE_GAME":"/gs2c/closeGame.do?symbol=vs20fparty2","region":"Other","jurisdictionRequirements":"CLK"}',
            mgckey: "stylename@generic~SESSION@alkapon-108556-8a68b531-bbdc-441d-81fa-f8060c282091",
            jurisdictionMsg: "",
            extendSessionUrl: "",
            extendSessionInterval: null
        });

        var lobbyurl = "https://rosh.bet/slots"
